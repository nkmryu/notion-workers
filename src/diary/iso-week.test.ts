...
import { formatIsoWeekTitle, getJstIsoWeek } from "./iso-week";
...
describe("formatIsoWeekTitle", () => {
  it("ISO 週番号を2桁でゼロ埋めする", () => {
    ...
    expect(formatIsoWeekTitle(getJstIsoWeek(new Date("2026-01-29T03:00:00.000Z")))).toBe(
      "26.W05",
    );
  });
  ...
  it("ISO 週基準年をタイトルの年に使う", () => {
    ...
    expect(formatIsoWeekTitle(getJstIsoWeek(new Date("2027-01-01T03:00:00.000Z")))).toBe(
      "26.W53",
    );
  });
});
