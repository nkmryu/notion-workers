...
import {
  buildRefsSection,
  collectMonthlyRefs,
  planMonthlyFinalizationSteps,
  shouldGenerateMonthlyRefs,
} from "./monthly-refs";

describe("collectMonthlyRefs", () => {
  it("本文リンク・bookmark・embedのURLを初出順で重複なく集める", () => {
    ...
    expect(collectMonthlyRefs(markdown)).toEqual([
  ...
  it("URL文字列と同じアンカーテキストはタイトルとして採用しない", () => {
    ...
    expect(collectMonthlyRefs(`[  ${url}  ](${url})`)).toEqual([
  ...
  it("URL表示の初出後に現れる最初の本文アンカーを採用する", () => {
    ...
    expect(collectMonthlyRefs(`[${url}](${url})\n[記事タイトル](${url})`)).toEqual(
  ...
  it("対象URLが無ければ空配列を返す", () => {
    ...
    expect(collectMonthlyRefs("本文だけ\n<empty-block/>")).toEqual([]);
  });
});
...
describe("shouldGenerateMonthlyRefs", () => {
  const pastMonthly = {
  ...
  it("Refs見出しがあれば生成しない", () => {
    ...
    expect(
      shouldGenerateMonthlyRefs(pastMonthly, ["Refs"], now, true),
    ).toBe(false);
  });
  ...
  it("未ロックの過去月はロック可能になった場合だけ生成する", () => {
    ...
    expect(
      shouldGenerateMonthlyRefs(pastMonthly, [], now, false),
    ).toBe(false);
    expect(
      shouldGenerateMonthlyRefs(pastMonthly, [], now, true),
    ).toBe(true);
  });
  ...
  it("今月のMonthlyは生成対象にしない", () => {
    ...
    expect(
      shouldGenerateMonthlyRefs(
        { ...pastMonthly, currentTitle: "26.M07" },
        [],
        now,
        true,
      ),
  ...
  it("Refsの無いロック済み過去月はロック計画と無関係に生成対象にする", () => {
    ...
    expect(shouldGenerateMonthlyRefs(locked, [], now, false)).toBe(true);
    expect(shouldGenerateMonthlyRefs(locked, ["Refs"], now, false)).toBe(false);
  });
});

describe("planMonthlyFinalizationSteps", () => {
  it("同じMonthlyのRefs生成をロックより前へ並べる", () => {
    // Refs追記が成功する前にMonthlyをロックしない実行順序を保証する。
    expect(planMonthlyFinalizationSteps(["monthly"], ["monthly"])).toEqual([
      { type: "generate_refs", pageId: "monthly" },
      { type: "lock", pageId: "monthly" },
    ]);
  });
});
