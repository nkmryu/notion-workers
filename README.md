...
2. 日次ページが存在する ISO 週に週次ページが無ければ、古い週から順にすべて作成し、タイトルを `26.W30` 形式にします。
3. 日次ページが存在する暦月に月次ページが無ければ、古い月から順にすべて作成し、タイトルを `26.M07` 形式にします。
4. 終了した日次ページの本文は、同じ ISO 週の未ロック週次ページへ日付見出し付きで転記します。転記はリネーム・ロックより先に実行するため、週が変わった直後も前週の日曜分を転記してから前週ページをロックします。
5. 同じ本文を同じ暦月の未ロック月次ページにも転記します。月が変わった直後は、前月の最終日を転記してから前月ページをロックします。
